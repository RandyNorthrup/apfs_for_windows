...
private:
    ...
    void changeSelectedMountMode() {
        ...
        const QString detail = readOnly
            ? QStringLiteral("Enable read/write mode for %1 (%2)? The service will verify the exact APFS target before enabling raw writes.")
                  .arg(mountName, target)
        ...
        if (readOnly && looksLikeRawDeviceTarget(target)) {
