...
param(
    ...
    [string]$ExpectedTarget = "\\.\PhysicalDrive2",
...
try {
    ...
    $writeProbe = if ($mountRoot) {
        Join-Path $mountRoot "apfs-autodiscovery-write-deny-$([guid]::NewGuid().ToString('N')).txt"
    } else {
        $null
    }
    ...
    $ok = $targetConfiguredFinal -and $hashMatches -and $writeDenied
