...
private:
    [[nodiscard]] bool ensureMountedAndScanned(PartitionApfsFileReadResult* result) {
        ...
        populateVolumePolicy(result);
        return mountedAndScanned_;
    }

    void populateVolumePolicy(PartitionApfsFileReadResult* result) const {
        result->volume_policy.known = volumePolicyKnown_;
        result->volume_policy.incompatible_features = volumeIncompatibleFeatures_;
        result->volume_policy.file_system_flags = volumeFileSystemFlags_;
        result->volume_policy.sealed =
            (volumeIncompatibleFeatures_ & kApfsVolumeIncompatSealed) != 0;
        result->volume_policy.encrypted = volumeEncrypted_;
        result->volume_policy.one_key = volumeEncrypted_ && volumeOneKey_;
        result->volume_policy.per_file_keys = volumeEncrypted_ && !volumeOneKey_;
    }
    ...
    [[nodiscard]] bool initializeVolumeState(const QByteArray& volumeBlock,
        ...
        volumeIncompatibleFeatures_ =
            le64(volumeBlock, kApfsVolumeIncompatibleFeaturesOffset);
        volumeFileSystemFlags_ = le64(volumeBlock, kApfsVolumeFsFlagsOffset);
        volumePolicyKnown_ = true;
        volumeCaseInsensitive_ =
            (volumeIncompatibleFeatures_ & kApfsVolumeIncompatCaseInsensitive) != 0;
        if ((volumeIncompatibleFeatures_ & kApfsVolumeIncompatIncompleteRestore) != 0) {
            result->blockers.append(QStringLiteral("APFS volume has incomplete-restore state"));
            return false;
        }
        if ((volumeIncompatibleFeatures_ & ~kApfsSupportedVolumeIncompat) != 0) {
            result->blockers.append(
                QStringLiteral("APFS volume has unsupported incompatible features 0x%1")
                    .arg(volumeIncompatibleFeatures_, 0, kDisplayHexBase));
            return false;
        }
    ...
    [[nodiscard]] bool unlockEncryptedVolume(const QByteArray& volumeBlock,
                                             PartitionApfsFileReadResult* result) {
        const uint64_t fsFlags = volumeFileSystemFlags_;
        ...
        volumeEncrypted_ =
            (fsFlags & kApfsVolumeFsFlagUnencrypted) == 0 && keylockerPaddr_ != 0;
        if (!volumeEncrypted_) {
            return true;
        }
    ...
    bool volumePolicyKnown_{false};
    bool volumeEncrypted_{false};
    bool volumeOneKey_{false};  // ONEKEY whole-volume FileVault: data tweak = block addr
    uint64_t volumeIncompatibleFeatures_{0};
    uint64_t volumeFileSystemFlags_{0};
